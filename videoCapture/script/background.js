...
importScripts("../js/initialize.js");
...
chrome.alarms.onAlarm.addListener(function (alarm) {
    console.log(" chrome.alarms.onAlarm ", alarm);
});
...
chrome.webRequest.onSendHeaders.addListener(
    function (details) {
        ...
        console.log(" chrome.webRequest.onSendHeaders ");
    },
    {urls: ["<all_urls>"]}, // 过滤器：监听所有 URL
    ["requestHeaders", "extraHeaders"] // 额外信息：需要请求头数据
);

/**
 * onResponseStarted 监听所有网站网络请求的「响应开始阶段」（服务器开始返回数据）
 * @author LiuQi
 */
chrome.webRequest.onResponseStarted.addListener(function () {
        console.log(" chrome.webRequest.onResponseStarted " + G.name);
    }, {urls: []}, // 过滤器：监听所有 URL
    [] // 额外信息：需要请求头数据
);
